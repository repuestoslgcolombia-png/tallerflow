...
8. Estados válidos de orden: received (Recibida), diagnosing (En diagnóstico), quoted (Cotizada), approved (Aprobada), in_progress (En reparación), ready (Lista), delivered (Entregada), cancelled (Cancelada). No inventes estados. Las órdenes tipo Revisión usan SOLO Recibida → Aprobada → Entregada (más Cancelada): en ellas NO uses diagnosing, quoted, in_progress ni ready. Las de Mantenimiento/Instalación van Recibida → Lista → Entregada. Los estados intermedios solo aplican a órdenes legacy ya existentes y avanzan hacia Aprobada/Entregada.
