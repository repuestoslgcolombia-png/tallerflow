...
import { CreateQuoteDialog } from '@/modules/quotes/create-quote-dialog'
...
function StatusStepper({
  ...
  const isOutOfFlow = currentIdx === -1
  const statusConf = WORK_ORDER_STATUS[statusKey]

  return (
    <ol className="space-y-0">
      {isOutOfFlow && statusConf && (
        <div className="mb-2 p-2 rounded bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800">
          <p className="text-xs text-amber-700 dark:text-amber-400 flex items-center gap-1.5">
            <AlertTriangle className="size-3.5" />
            Estado <span className="font-mono">{statusKey}</span> fuera del flujo de este servicio (etapas: {stages.join(' → ')}).
          </p>
        </div>
      )}
      <ol className="space-y-0">
